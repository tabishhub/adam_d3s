...
def TikhonovDiff(f, dx, lam, d=1):
    ...
    g = np.squeeze(
        np.asarray(
            np.linalg.lstsq(A.T.dot(A) + lam * D.T.dot(D), A.T.dot(f), rcond=None)[0]
        )
    )
...
def build_linear_system(
    ...
    if time_diff == "FDconv":
        ...
        for i in range(n2):
            ut[i, :] = PolyDiff(
                u[i + offset_x, :], T, diff=1, width=width_t, deg=deg_t
            )[:, 0]
...
def print_pde(w, rhs_description, ut="u_t"):
    ...
    for k in range(11):
        s = abs(w) < 0.04  # find coefficients less than eps ...
        w = w.at[s].set(0.0)  # ... and set them to zero
    pde = ut + " = "
    first = True
    ...
    for forward_iter in range(maxit_f):
        ...
        for i in zero_coeffs:
            ...
            if relearn_f:
                ...
                w_added[list(F_trial)] = np.linalg.lstsq(
                    X[:, list(F_trial)], y, rcond=None
                )[0]
            ...
            for backward_iter in range(maxit_b):
                ...
                for j in non_zeros:
                    ...
                    if relearn_b:
                        ...
                        w_simple[list(F_trial)] = np.linalg.lstsq(
                            X[:, list(F_trial)], y, rcond=None
                        )[0]
